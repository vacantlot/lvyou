...
<%
    ...
    String user_name = (String)request.getAttribute("userId"); //用户名查询关键字
...
<jsp:include page="../header.jsp"></jsp:include>
	<div class="col-md-9 wow fadeInLeft">
		<ul class="breadcrumb">
  			...
  			<li><a href="<%=basePath %>user/frontlist">用户信息列表</a></li>
			...
			<div class="col-md-3 bottom15" <%=clearLeft %>>
			  <%--<a  href="<%=basePath  %>UserInfo/<%=userInfo.getUser_name() %>/frontshow"><img class="img-responsive" src="<%=basePath%><%=userInfo.getUserPhoto()%>" /></a>--%>
			     <div class="showFields">
			        ...
			        <a class="btn btn-primary top5" href="<%=basePath %>user/<%=userInfo.getUserId() %>/frontshow">详情</a>
			        <a class="btn btn-primary top5" onclick="userInfoEdit('<%=userInfo.getUserId() %>');" style="display:none;">修改</a>
			        <a class="btn btn-primary top5" onclick="userInfoDelete('<%=userInfo.getUserId() %>');" style="display:none;">删除</a>
			     </div>
			</div>
			<%  } %>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="userInfoQueryForm" id="userInfoQueryForm" action="<%=basePath %>UserInfo/frontlist" class="mar_t15" method="post">
			<div class="form-group">
				...
				<%--<input type="text" id="user_name" name="user_name" value="<%=user_name %>" class="form-control" placeholder="请输入用户ID">--%>
				<input type="text" id="user_name" name="user_name" value="" class="form-control" placeholder="请输入用户ID">
			</div>
			<div class="form-group">
				<label for="name">姓名:</label>
				<%--<input type="text" id="name" name="name" value="<%=name %>" class="form-control" placeholder="请输入姓名">--%>
				<input type="text" id="name" name="name" value="" class="form-control" placeholder="请输入姓名">
			</div>
			<div class="form-group">
				...
				<input type="text" id="birthday" name="birthday" class="form-control"  placeholder="请选择出生日期" value="" onclick="SelectDate(this,'yyyy-MM-dd')" />
			</div>
